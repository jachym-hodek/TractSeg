...
### FINAL NOTE: Before running everything, I recommend checking the Config class in base.py again, so all the variables are set correctly and make sure that .tractseg folder is in your home - ~ - directory, do also note that the ~ directory can be different for normal interface and running a job, so you should check where is this directory when running the job
